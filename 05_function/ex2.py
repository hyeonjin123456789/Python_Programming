...
def swap(a, b):  # 매개변수는 객체의 참조값을 전달 (같은 객체를 기리킴)
    a, b = b, a
    print(a, b)  # 2, 1 -> 함수 종료 시 지역변수 a, b 유호하지 않게 됨
...
# 1?? n이 짝수면 True, 홀수면 False를 반환하는 함수 작성하기
...
# 2?? 가변 인자로 여러 숫자를 받아 (최소값, 최대값, 합계, 평균) 튜플 리턴하기
...
# 3?? 이름과 키워드 가변 인자로 받은 정보로 아래와 같이 문자열을 만들어 리턴하기
