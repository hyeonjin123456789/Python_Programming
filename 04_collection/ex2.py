...
hyeonjin = [x ** 2 for x in range(1,11)]
print(hyeonjin)

# 1 ~ 10 중 짝수의 제곱수로 된 리스트 만들기 (필터링 if문 추가)
hyeonjin = [x ** 2 for x in range(1, 11) if x % 2 == 0]
print(hyeonjin)

# 1 ~ 10 중 짝수면 "짝", 홀수면 "홀" 출력하기
hyeonjin = ["짝수" if x % 2 == 0 else "홀수" for x in range(1,11)]
print(hyeonjin)

# 각 이름의 길이로 이루어진 리스트 만들기
names = ["pororo", "crong", "poby", "eddy"]
hyeonjin = [len(x) for x in names]
print(hyeonjin)

# 길이가 5 이상인 이름만 뽑기
hyeonjin = [len(x) for x in names if len(x) >= 5]
print(hyeonjin)
...
result = [x for x in scores if "n" in x]  
...
result = [round(sum(score) / len(score), 2) for score in zip(str1, str2, str3)]
